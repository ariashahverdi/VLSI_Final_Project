...
void main(int argc , char* argv[])
{
	...
	while(init_temp > 0){
		...
		//polish_exp_new = getRandom(module_count);
			...
			c = getchar( );
			cost_new.area = get_area(module_count, cost, module_array,polish_exp);
			printf("< %f, %f >\n",cost_new.area, cost.area);
			save_design(module_count, cost, module_array,polish_exp);
#endif
#ifdef DEBUG
			printf("** 2 **\n");
#endif
		...
		save_design(module_count, cost, module_array,polish_exp_new);
		printf( "Enter Something, Not That We Care Tho :P");
		c = getchar( );
		cost_new.area = get_area(module_count, cost, module_array,polish_exp_new);
		printf("< %f, %f >\n",cost_new.area, cost.area);
